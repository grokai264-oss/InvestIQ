"""NSE delivery + FII/DII with in-memory TTL cache."""
...
class NSEFetcher:
    ...
    def fetch_delivery_data(self, symbol: str) -> float:
        ...
        if self._session is None: return 0.0
        try:
            r = self._session.get(
            ...
            pct = r.json().get("securityWiseDP", {}).get("deliveryToTradedQuantity", 0.0)
    ...
    def fetch_fii_dii_data(self) -> Dict[str, float]:
        ...
        if self._session is None: return empty
        try:
